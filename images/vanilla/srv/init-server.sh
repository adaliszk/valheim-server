...
if [[ ! -d "${SERVER_PATH}/valheim_server_Data" ]];
  then
    echo -e "Extracting Server files in ${SERVER_PATH}"
    ...
    echo -e "Extracting Server files took $(bc -l <<< "(${EXTRACT_END}-${EXTRACT_BEGIN})*1000")ms"
  else
    echo -e "Server files already extracted, skipping..."
  fi
...
if [ -f "${WORLD_FILE}" ];
  then
    echo -e "World \"${SERVER_WORLD}\" is $(stat --printf="%s" "${WORLD_FILE}") bytes large"
    echo -e "Worlds are $(du --bytes "${DATA_PATH}/worlds" | cut -f1) bytes large"
  fi
...
log-env SERVER_NAME SERVER_PASSWORD SERVER_WORLD SERVER_PUBLIC SERVER_CONSOLE
log-env SERVER_ADMINS SERVER_PERMITTED SERVER_BANNED

function list-to-file {
  if [[ -n "${1}" ]];
    then
      echo -e "Saving \"${1}\" into ${2}"
      echo "${1}" | tr ',' '\n' > "${DATA_PATH}/${2}"
    fi
}
